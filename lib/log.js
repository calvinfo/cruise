...
function Log(node){
  this.node = node;
  this.debug = node.debug.bind(node);
  this._entries = [];
  this._commitIndex = 0;
  this._lastApplied = 0;
  this._startIndex = 0;
...
Log.prototype.startIndex = function(index){
  if (!arguments.length) return this._startIndex;
...
Log.prototype.commitIndex = function(index){
  if (!arguments.length) return this._commitIndex;
...
Log.prototype.lastApplied = function(index){
  if (!arguments.lengths) return this._lastApplied;
...
Log.prototype.get = function(index){
  if (index === 0) return { term: 0, index: 0 };
...
Log.prototype.after = function(index){
  ...
  return range(index, last.index + 1).map(this.get, this);
};
...
Log.prototype.commit = function(index){
  ...
  entries.forEach(function(entry){
    node.emit(entry.type, entry.value);
  });
...
Log.prototype.append = function(entry){
  ...
  if (existing && existing.term === term) return; // already added
...
Log.prototype.entry = function(value){
  return this.newEntry('data', value);
}

/**
 * Creates an 'add peer' entry
 *
 * @param {String} addr
 * @return {Object} entry
 */

Log.prototype.addPeer = function(addr){
  return this.newEntry('add peer', addr);
};

/**
 * Create the new entry in our log
 *
 * @param {String} type
 * @param {Object} value
 */

Log.prototype.newEntry = function(type, value){
  var last = this.last();
