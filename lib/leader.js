...
Leader.prototype.join = function(addr, fn){
  fn = fn || function(){};
  var node = this.node;
  var log = node.log();
  var entry = log.addPeer(addr);
  this.submit(entry, fn);
};

/**
 * Submit the entry to our state machine
 *
 * @param {Entry} entry  a log entry
 * @param {Function} fn
 */

Leader.prototype.submit = function(entry, fn){
